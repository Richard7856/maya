// Contexto de autenticación para la app del personal de limpieza.
// Responsabilidades:
//   1. Mantener la sesión de Supabase y el perfil del usuario
//   2. Sincronizar el JWT con el api-client vía setAuthToken()
//   3. Validar que el rol sea 'cleaning' — rechaza inquilinos/otros que intenten usar esta app
import {
  createContext,
  useContext,
  useEffect,
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function signIn(email: string, password: string) {
    ...
    // Validate role before granting access — cleaning staff only
    setAuthToken(data.session.access_token);
    const p = await usersApi.me();
    if (p.role !== "cleaning") {
      await supabase.auth.signOut();
      throw new Error("Esta app es exclusiva para personal de limpieza.");
    }
    setProfile(p);
  }
